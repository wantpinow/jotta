...
model = Cls.lookup(f"{APP_PREFIX}-llama-3", "Model", environment_name=ENVIRONMENT)
...
questions = [
    ...
    "Return a set of lessons plans for a spanish class as a JSON list. Each lesson plan should have a title, a description, and a list of activities.",
]
...
print(
    f"Batch of {len(questions)} predictions took {end_time - start_time:.2f} seconds (average {1000 * (end_time - start_time) / len(questions):.2f} ms)"
)
print()
